...
void CGrid::Serialize(CArchive &ar)
{
	if( ar.IsStoring() ) {
        ...
        ar	>> m_strID ;
		if( m_strID.Find(_T("_gridMod3D"), 0) == -1 )  {
			SerializeOld(ar);
			return;
		}
...
BOOL CGrid::Open()
{
	...
	CFileDialog dlg(TRUE, _T("grd"), _T("*.grd"), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("Surfer 6.0 Grid (*.grd)|*.grd|Surfer 7.0 Grid (*.grd)|*.grd|Geosoft Grid (*.grd)|*.grd||"));
	if(dlg.DoModal() == IDOK) {
...
BOOL CGrid::Open(CString strFilePath, int nType)
{
	...
	if( m_nType == GEOS_KX1 )	{// geosoft default kx = 1;
		AfxMessageBox(_T("Not available"));
		return FALSE;
	}
	...
	while(file.Read(&nId, 4)) {
		...
		if(nId == 0x41544144) {
			...
			if(!m_bAllocated) {
				...
				AfxMessageBox(_T("Grid allocating problem!"));
				file.Close();
				Close();
				return FALSE;
			}
	...
	if( file.Open(strFilePath, CFile::modeRead) ) {
		...
		if(strstr(id, "DSBB") != NULL) {
			...
			if(!m_bAllocated) {
				...
				AfxMessageBox(_T(" grid alocating problem!"));
				return FALSE;
			}
		}
		else {
			AfxMessageBox(_T("Grid type does not match."));}
	}
	return FALSE;
}
...
BOOL CGrid::CreateContourLevels(void)
{
	...
	UpdateMinMaxValue();
	if( m_dMaxZ == m_dMinZ ) {
		AfxMessageBox(_T("Grid is planar image creation skipped."), MB_OK | MB_ICONINFORMATION);
		return 0;
	}
...
BOOL CGrid::DrawContours(CDC* pDC, double scx, double scy, double offx, double offy)
{
	if(m_bAllocated /*&& m_bShowContour*/) {
		...
		TRACE(_T("Trying to draw an empty grid in CGrig.Draw(). \n"));
	}
	return FALSE;
}
...
int CGrid::CreateImage(CImage &img, int nWidth, int nHeiht, int nTag)
{
	...
	if(img.Create(nWidth, nHeiht, 32) ) {
		...
		AfxMessageBox(_T("Problem creating Image."));
	}
...
BOOL CGrid::GetGL(DoubleArray* pData, int nFormat, double min, double max)
{
	...
	if(m_nRow<3 || m_nCol<3) return 1;

	double dc = (m_xSize*m_nCol/2.0+m_ySize*m_nRow/2.0) / (m_dMaxZ-m_dMinZ);
	int m = (int)m_clrTbl.GetSize();
	for(int i = 0; i < (int)m_nRow; i++) {
		for(int j = 0; j < (int)m_nCol; j++) {
			...
			if(i<1) k=1;
			if(j<1) l=1;
			if(i>(int)m_nRow-2) k=m_nRow-2;
			if(j>(int)m_nCol-2) l=m_nCol-2;
			//if(i<1 || i>(int)(m_nRow-2) || j<1 || j>(int)(m_nCol-2) ) {
			//	nx = 0.0f;
			//	ny = 0.0f;
			//	nz = -1.0f;
			//}
			//else {
				...
				mz = dc*m_pData[k][l];
				az = dc*m_pData[k-1][l-1];
				bz = dc*m_pData[k-1][l+1];
				cz = dc*m_pData[k+1][l+1];
				dz = dc*m_pData[k+1][l-1];

				double nx1, ny1, nz1;
				double nx2, ny2, nz2;
				double nx3, ny3, nz3;
				double nx4, ny4, nz4;
				Nornal(mx-bx, my-by, mz-bz, ax-mx, ay-my, az-mz, nx1, ny1, nz1);
				Nornal(mx-cx, my-cy, mz-cz, bx-mx, by-my, bz-mz, nx2, ny2, nz2);
				Nornal(mx-dx, my-dy, mz-dz, cx-mx, cy-my, cz-mz, nx3, ny3, nz3);
				Nornal(mx-ax, my-ay, mz-az, dx-mx, dy-my, dz-mz, nx4, ny4, nz4);

				nx = (nx1+nx2+nx3+nx4)/4.0;
				ny = (ny1+ny2+ny3+ny4)/4.0;
				nz = (nz1+nz2+nz3+nz4)/4.0;
			//}

			pData->SetAt(nIndex + 3, nx);			// nx
			pData->SetAt(nIndex + 4, ny);			// ny
			pData->SetAt(nIndex + 5, nz);			// nz
...
int	CGrid::CreateImageLegeng(CImage &img, int nWidth, int nHeiht, int nTag)
{
	...
	if( img.Create(nWidth, nHeiht, 32) ) {
		...
		AfxMessageBox(_T("Problem creating legeng image."));
	}
...
int CGrid::CreateColorTableGL(float* fA, int n, int nFormat, float fAlpha)
{
	// pTbl pointer to array of 1D texture
	// number of elements in pTbl = n*nFormat; nFormat must be 3, 4
	// n MUST be power of 2
	...
	m_clrGrad.CreateColorTableGL(fA, n, nFormat, fAlpha, m_bHistClr);
...
int CGrid::ReadType(CString strFilePath)
{
	...
	if( file.Open(strFilePath, CFile::modeRead) ) {
		...
		AfxMessageBox(_T("Unknown file type."), MB_OK | MB_ICONINFORMATION);
		file.Close();
	}
	else {
		AfxMessageBox(_T("Failed to open grid file"), MB_OK | MB_ICONINFORMATION);
	}
	return -1;
}
...
{
	CFileDialog dlg(TRUE, _T("bln"), _T("*.bln"), OFN_HIDEREADONLY, _T("Blanking text file (*.bln)|*.bln||"));
	//dlg.SetWindowText(_T("Open Blanking File"));
	...
	while( file.ReadString(strLine) ) {
		if( !strLine.IsEmpty() && bData ) {
			...
			nItems = sscanf_s(CT2A(strLine), "%f %f %f %f %f", d, d + 1, d + 2, d + 3, d + 4);
			if( nItems >1 ) {
				plg.Add(*d);
				plg.Add(*(d+1));
			}
		...
		if( !strLine.IsEmpty() && !bData ) {
			strLine.Replace(_T(","), _T(" "));
			nItems = sscanf_s(CT2A(strLine), "%d %d", &len, &tag);
			bData = TRUE;
		}
	}
	file.Close();
	...
	int n = Blank(plg.GetData(), (int)plg.GetSize()/2, 2, tag);

	CString strRes;
	strRes.Format(_T("%d nodes blanked"), n);
