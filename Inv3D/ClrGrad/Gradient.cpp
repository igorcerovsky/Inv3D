...
void CGradient::SortPegs() {QuickSort(0, (int)pegs.GetUpperBound());}
int CGradient::GetPegCount() const {return (int)pegs.GetSize();}
...
int CGradient::SetPeg(int iIndex, COLORREF crColour, float fPosition)
{
	...
	if(iIndex == STARTPEG) {
		...
		m_StartPeg.position = fPosition;
	}
	else if(iIndex == ENDPEG)  {
		m_EndPeg.colour = crColour;
		m_EndPeg.position = fPosition;
	}
...
void CGradient::ColourFromPosition(float pos, BYTE& r, BYTE& g, BYTE& b)
{
	COLORREF c;

	c = ColourFromPosition(pos);
	r = GetRValue(c);
	g = GetGValue(c);
	b = GetBValue(c);
}

void CGradient::ColourFromPosition(float pos, float& r, float& g, float& b)
{
	COLORREF c;
...
COLORREF CGradient::ColourFromPosition(float pos)
{
	...
	if(pegs.GetSize() > 0)
	{
		...
		if(pos <= firstpegpos)
			return Interpolate(m_StartPeg.colour, pegs[0].colour, pos, 0, firstpegpos);
		else if(pos > lastpegpos)
			return Interpolate(pegs[pegs.GetUpperBound()].colour, m_EndPeg.colour, pos, lastpegpos, 1);
		else
		{
			int curpeg;
			curpeg = IndexFromPos(pos);
			return Interpolate(pegs[curpeg].colour, pegs[curpeg+1].colour, pos, pegs[curpeg].position, pegs[curpeg+1].position);
		}
	}
	else
	{
...
void CGradient::Serialize(CArchive &ar)
{
	...
	else
	{
		...
		for(int i = 0; i < (int)pegs.GetSize(); i++)
			pegs[i].Serialize(ar);
	}
}
...
int CGradient::CreateColorTableGL(float* fA, const int n, const int nFormat, const float fAlpha, const int bHst)
{
	// fA pointer to array of 1D texture
	...
	double		d, dh;
	float		fr, fg, fb;
	int nHst = (int) m_histogram.GetCount();

	for(int i=0; i < n; i++) {
		if(!m_histogram.IsEmpty() /*&& nHst>=n*/ && bHst) {
			dh = (double)(nHst-1)/(n-1);
			d = m_histogram[(int)(i*dh)];
		}
		else {
			d = (double) (i)/(n-1);
		}
		ColourFromPosition((float)d, fr, fg, fb);

		fA[nFormat*i + 0] = fr/255.0f;
		fA[nFormat*i + 1] = fg/255.0f;
		fA[nFormat*i + 2] = fb/255.0f;
		if(nFormat==4)	fA[nFormat*i + 3] = fAlpha;
	}

	return 1;
};

double CGradient::GetHistogramValue(double val, BOOL bHst)
{
	int		n, i;
	double	ret;

	//ASSERT(val>=m_dHstMin || val<=m_dHstMax);
	...
	if( bHst ) {
		n = (int)m_histogram.GetCount();
		i = (int) floor((n-1)*(val - m_dHstMin) / (m_dHstMax-m_dHstMin));
		ASSERT(i<n);
		ret =  m_histogram[i];
	}
	// linear color distribution
	else {
		ret = (val - m_dHstMin) / (m_dHstMax-m_dHstMin);
		if(ret<0.0) ret = 0;
		if(ret>1.0) ret = 1.0;
	}
