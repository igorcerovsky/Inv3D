...
void CGradientCtrlImpl::Draw(CDC *dc)
{
	...
	if(IsVertical())
	{
		...
		dc->FrameRect(CRect(m_LeftDownSide ? 23 : 4, 4, GetDrawWidth() - (m_RightUpSide ? 23 : 4),
...
void CGradientCtrlImpl::DrawGradient(CDC *dc)
{
	...
	// !!!change
	int w = drawwidth - (m_RightUpSide ? 24 : 5) - (m_LeftDownSide ? 24 : 5);	//!!!change
	//int wRc = drawwidth - (m_RightUpSide ? 24 : 5) - (m_LeftDownSide ? 24 : 5);	//!!!change
	//int w=10;
	...
	if(vertical) {
		...
		DrawHistogram(dc, m_LeftDownSide ? 24 : 5, 5, w, l);
	}
	else {
		dc->StretchBlt(5, clientrect.bottom - drawwidth + (m_RightUpSide ? 24 : 5), l, w, &memdc, 0, 0, l, 1, SRCCOPY);
		DrawHistogram(dc, 5, clientrect.bottom - drawwidth + (m_RightUpSide ? 24 : 5), l, w);
	}
...
void CGradientCtrlImpl::DrawHistogram(CDC* pDC, int left, int top, int width, int height)
{
	CPen pen, *pPen;
	pen.CreatePen(PS_SOLID, 0, RGB(80, 80, 80));
	DoubleArray* pHst = m_Owner->m_Gradient.GetHistogram();

	int n;
	int x, y;
	double scx, scy, ofx, ofy;
...
void CGradientCtrlImpl::DrawSelPeg(CDC *dc, int peg)
{
	...
	if(peg == STARTPEG)
	{
		if(m_RightUpSide)
			...
			else dc->Rectangle(5, clientrect.bottom-drawwidth-9, 10,	clientrect.bottom-drawwidth-14);
